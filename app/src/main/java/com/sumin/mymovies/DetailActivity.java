...
//у некоторых фильмоф нет постера
...
// трейлеры открываются в ютубе
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intent != null && intent.hasExtra("id")) {
            ...
            Log.d("eee", String.valueOf(id));
        } else {
            finish();
        }
        viewModel = ViewModelProviders.of(this).get(MainViewModel.class);
        Log.d("eee", viewModel.getMovieByTitle(viewModel.getMovieById(id).getTitle()).getTitle());
        movie = viewModel.getMovieById(id);
        Log.d("eee",String.valueOf(movie.getBigPosterPath()));
