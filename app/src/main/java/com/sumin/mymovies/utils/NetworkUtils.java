...
public class NetworkUtils {
    ...
    private static final String API_KEY = "0043e796c5d76975f5b698d0f53134e8";
    private static final String LANGUAGE_VALUE = "en-EN";
    private static final String SORT_BY_POPULARITY = "popularity.desc";
    private static final String SORT_BY_TOP_RATED = "vote_average.desc";
    private static final String MIN_VOTE_COUNT_VALUE = "10000";
    ...
    public static URL buildURLToSearch(String title){
        Uri uri = Uri.parse(BASE_URL_SEARCH).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY,API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE,LANGUAGE_VALUE)
                .appendQueryParameter(PARAMS_QUERY,Uri.encode(title))
                .build();
        try {
            return new URL(uri.toString());
        } catch (MalformedURLException e) {
    ...
    public static JSONObject getJSONForSearch(String title){
        JSONObject result = null;
        URL url = buildURLToSearch(title);
        try {
            result = new JSONLoadTask().execute(url).get();
        } catch (ExecutionException e) {
            e.printStackTrace();
    ...
    // позволяет загружать даже после прерываения загрузки
    public static class JSONLoader extends AsyncTaskLoader<JSONObject> {
        ...
        //нужен чтобы менять статус начала загрузки
        @Override
