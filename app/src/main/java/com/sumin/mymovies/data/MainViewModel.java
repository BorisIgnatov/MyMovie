...
public class MainViewModel extends AndroidViewModel {
    ...
    public Movie getMovieByTitle(String title){
        try {
            return new GetMovieByTitleTask().execute(title).get();
        } catch (ExecutionException e) {
            e.printStackTrace();
        } catch (InterruptedException e) {
            e.printStackTrace();
    ...
    private static class GetMovieByTitleTask extends AsyncTask<String, Void, Movie>{
        @Override
        protected Movie doInBackground(String... strings){
            if(strings !=null && strings.length > 0){
                return database.movieDao().getMovieByTitle(strings[0]);
            }
            return null;
        }
    }
